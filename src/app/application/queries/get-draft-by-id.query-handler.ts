...
import { DraftApplication, GetDraftByIdQuery } from '@/domain/application';
import { UrlSignerService } from '@/domain/common';
import { CompanyDocument, DraftApplicationDocument } from '@/infra/adapters';
import { ExtendedAuthContextStorage, hasPermission, Permission } from '@/infra/common';

import { DraftResponse } from '../dtos';

@QueryHandler(GetDraftByIdQuery)
export class GetDraftByIdQueryHandler extends BaseQueryHandler<GetDraftByIdQuery, DraftResponse> {
  constructor(
  ...
  handle(query: GetDraftByIdQuery) {
    ...
    return of(query).pipe(
      ...
      mergeMap(({ _id }) =>
        this.model
          .aggregate<DraftApplicationDocument>()
          .match(query.onlyMine ? { _id, created_by: new Types.ObjectId(this.context.store.userId) } : { _id })
          .lookup({
      ...
      map(([draft]) => draft),
      throwIfVoid(() => NotFound.of(DraftApplication, query.id)),
      tap((app: DraftApplicationDocument & { company: CompanyDocument }) => {
        // Flatten emails
        Object.assign(
          app.company.emails,
        ...
        if (app.signature_url) {
          Object.assign(app, { signature_url: this.signer.sign(`${appURI}/${app.signature_url}`) });
        }
      ...
      map((app) =>
        plainToInstance(DraftResponse, app, {
