...
import { Application, ApplicationRepository, RestoreBankNotificationCommand } from '@/domain/application';
import { ApplicationBlocked, Id } from '@/domain/common';

@CommandHandler(RestoreBankNotificationCommand)
export class RestoreBankNotificationCommandHandler extends BaseCommandHandler<RestoreBankNotificationCommand, void> {
  constructor(
  ...
  handle(command: RestoreBankNotificationCommand): Observable<void> {
    return of(command).pipe(
      mergeMap((command) => this._getApplication(command.applicationId)),
      tap((application: Application) => application.restoreNotification(command.notificationId).getOrThrow()),
