...
import { IdentityProviderRepository, RemoveRoleFromUserCommand, User, UserRepository } from '@/domain/user';

@CommandHandler(RemoveRoleFromUserCommand)
export class RemoveRoleFromUserCommandHandler extends BaseCommandHandler<RemoveRoleFromUserCommand, void> {
  constructor(
      ...
      delayWhen(({ user, identityUserId }) =>
        this.transactionService.runInTransaction(() =>
          zip([
            ...
            this.identityProviderRepository.removeRole(identityUserId, command.role),
          ]),
        ),
      ),
      mapToVoid(),
