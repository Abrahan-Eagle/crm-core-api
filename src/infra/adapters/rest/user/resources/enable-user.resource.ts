...
import { EnableUserCommand } from '@/domain/user';
import { Permission, RequiredPermissions } from '@/infra/common';

@Controller('v1/users/enable/:user_id')
export class EnableUserResource extends AbstractHttpResource {
  @Put()
  ...
  handle(@Req() req: Request, @Res() res: Response, @Param('user_id') id: string): Observable<Response> {
    return of(id).pipe(
      map((id) => EnableUserCommand.create(id).getOrThrow()),
