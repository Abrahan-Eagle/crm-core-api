...
import { EnableUserCommand, IdentityProviderRepository, User, UserRepository } from '@/domain/user';

@CommandHandler(EnableUserCommand)
export class EnableUserCommandHandler extends BaseCommandHandler<EnableUserCommand, void> {
  constructor(
    ...
    return this.repository.findById(command.id).pipe(
      ...
      delayWhen(({ user, identityUserId }) =>
        this.transactionService.runInTransaction(() =>
          zip([this.repository.updateOne(user), this.identityProviderRepository.enableUser(identityUserId)]),
        ),
      ),
      mapToVoid(),
    );
